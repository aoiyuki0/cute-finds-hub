...
import type { CategoryId as _CategoryId, ComparisonGroupId as _ComparisonGroupId, Product as _Product, ProductId as _ProductId } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    constructor(private actor: ActorSubclass<_SERVICE>, private _uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, private _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, private processError?: (error: unknown) => never){}
    async getComparisonGroup(arg0: ComparisonGroupId): Promise<ComparisonGroup> {
        if (this.processError) {
            try {
                const result = await this.actor.getComparisonGroup(arg0);
                return result;
            } catch (e) {
                this.processError(e);
    ...
    async listComparisonGroups(): Promise<Array<ComparisonGroup>> {
        if (this.processError) {
            try {
                const result = await this.actor.listComparisonGroups();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function from_candid_opt_n3(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_ComparisonGroupId]): ComparisonGroupId | null {
...
function from_candid_record_n2(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: {
    id: _ProductId;
    title: string;
    comparisonGroup: [] | [_ComparisonGroupId];
    description: string;
    imageUrl: string;
    ...
    return {
        id: value.id,
        title: value.title,
        comparisonGroup: record_opt_to_undefined(from_candid_opt_n3(_uploadFile, _downloadFile, value.comparisonGroup)),
        description: value.description,
        imageUrl: value.imageUrl,
        category: value.category,
        rating: value.rating,
        priceHint: value.priceHint
    };
}
